...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  Installed {} flows",
        dnat_service.all_flows(10, 100).len()
    );
    ...
    conn.send_flow_sync(&rovs_openflow::Flow::delete().table(10))
        .await?;
    conn.send_flow_sync(&rovs_openflow::Flow::delete().table(11))
        .await?;
    conn.send_flow_sync(&rovs_openflow::Flow::delete().table(12))
        .await?;

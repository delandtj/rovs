...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !veth_exists {
        println!("\nNote: {} doesn't exist. Create it with:", VETH_NAME);
    ...
    // =========================================================================
    println!("\n=== Creating {} (external bridge) ===", BR_EXT);
    ...
    println!("  - Adding system port: {} (veth/physical NIC)", VETH_NAME);
    ...
    // =========================================================================
    println!("\n=== Creating {} (internal bridge) ===", BR_INT);
    ...
    println!("  - Patch port: patch-{}-to-{}", BR_EXT, BR_INT);
    println!("  - Patch port: patch-{}-to-{}", BR_INT, BR_EXT);
    ...
    println!("\n=== ovs-vsctl show ===\n");
    let db_arg = if addr.starts_with("unix:") {
        format!("--db={}", addr)
    } else {
        format!("--db={}", addr)
    };
    ...
    println!("\nTo clean up:");
    println!("  ovs-vsctl {} del-br {}", db_arg, BR_EXT);
    println!("  ovs-vsctl {} del-br {}", db_arg, BR_INT);
...
async fn commit_and_wait(
    ...
    match client.commit(txn).await {
        Ok(true) => {
            println!("  ✓ {} succeeded", description);
            client.wait().await?;
            Ok(())
        }
        Ok(false) => Err(format!("{} failed", description).into()),
        Err(e) => Err(format!("{} error: {}", description, e).into()),
    }
}
